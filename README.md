...
1.2.10 修复唤醒时屏幕暂不可用造成的恢复请求丢失：短时等待屏幕枚举，同一显示器重新可用后也能恢复；隐藏或终态停止会取消等待。截图或区域捕获回退后，点击「重连」会受控重试优选后端，普通自动重连仍保留回退，避免持续错误下反复切换。权限拒绝、用户停止和系统停止仍不会自动开启采集。

宠物隐藏且设置窗口关闭或最小化时暂停 Codex 探测；再次显示立即检测，保留主动隐藏前的任务计时和结果去重。设置窗口仍可见时继续联动。桌面端连续连接失败采用 1/2/4/8/16/30 秒退避（最多 30 秒），新鲜本地状态文件仍优先，Codex 启动通知可立即唤醒检测。没有引入持久 WebSocket，也没有改变正常联动的约一秒节奏。任务在隐藏期间是否曾结束并重新开始，仍受桌面状态快照本身不提供任务 ID 的限制。
