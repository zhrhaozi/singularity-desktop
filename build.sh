...
bash scripts/build-physics.sh
"${SWIFT[@]}" -swift-version 5 -O -target arm64-apple-macosx13.0 -import-objc-header PhysicsBridge.h main.swift Behavior.swift CodexState.swift RenderCache.swift MultiLensRenderer.swift OrbitalSystem.swift NativeSelfTest.swift .build/physics/libSingularityPhysics.a -o "$DEST/奇点.app/Contents/MacOS/Singularity" -framework Cocoa -framework SwiftUI -framework ScreenCaptureKit -framework OpenGL 2> build.log
cp Resources/* "$DEST/奇点.app/Contents/Resources/"
cp Vendor/Chipmunk2D/LICENSE.txt "$DEST/奇点.app/Contents/Resources/Chipmunk2D-LICENSE.txt"
cat > "$DEST/奇点.app/Contents/Info.plist" <<'PLIST'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict><key>CFBundleExecutable</key><string>Singularity</string><key>CFBundleIdentifier</key><string>local.singularity.pet</string><key>CFBundleName</key><string>奇点</string><key>CFBundleDisplayName</key><string>奇点</string><key>CFBundleVersion</key><string>16</string><key>CFBundleShortVersionString</key><string>1.3.0</string><key>CFBundlePackageType</key><string>APPL</string><key>LSMinimumSystemVersion</key><string>13.0</string><key>NSHighResolutionCapable</key><true/><key>CFBundleIconFile</key><string>AppIcon</string><key>NSScreenCaptureUsageDescription</key><string>奇点需要读取屏幕画面，以实时扭曲黑洞背后的桌面内容。画面仅在本机用于渲染，不录制或上传。</string></dict></plist>
PLIST
SIGNING_IDENTITY="${SINGULARITY_SIGNING_IDENTITY:-}"
if [ -z "$SIGNING_IDENTITY" ]; then
  SIGNING_IDENTITY="$(security find-identity -v -p codesigning 2>/dev/null | sed -n 's/.*"\(Apple Development:.*\)"$/\1/p' | head -n 1 || true)"
fi
if [ -n "$SIGNING_IDENTITY" ]; then
  echo "Signing with Apple Development identity: $SIGNING_IDENTITY"
  codesign --force --deep --sign "$SIGNING_IDENTITY" "$DEST/奇点.app"
else
